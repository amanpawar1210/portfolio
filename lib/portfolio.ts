...
export type Project = { id: string; title: string; category: string; summary: string; stack: string[]; url: string; theme: "ember" | "violet" | "mint" };
...
export async function readPortfolio(): Promise<Portfolio> {
  try {
    if (!env.DB) return starterPortfolio;
    const row = await env.DB.prepare("SELECT data FROM portfolio_content WHERE id = 1").first<{ data: string }>();
    return row?.data ? { ...starterPortfolio, ...JSON.parse(row.data) } : starterPortfolio;
  } catch (error) { console.error("Portfolio content unavailable", error); return starterPortfolio; }
}
...
export function cleanPortfolio(input: unknown): Portfolio {
  ...
  const projects = Array.isArray(data.projects) ? data.projects.slice(0, 24).map((item, index) => ({ id: text(item?.id, 60) || `project-${index}`, title: text(item?.title, 120), category: text(item?.category, 100), summary: text(item?.summary, 1200), stack: list(item?.stack), url: safeUrl(item?.url), theme: (["ember", "violet", "mint"].includes(item?.theme) ? item.theme : "ember") as Project["theme"] })) : [];
...
function safeUrl(value: unknown) { if (typeof value !== "string" || !value.trim()) return ""; try { const url = new URL(value); return ["https:", "http:"].includes(url.protocol) ? url.toString() : ""; } catch { return ""; } }
