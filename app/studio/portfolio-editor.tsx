...
import { ArrowLeft, ArrowUpRight, Check, CirclePlus, FileUp, LogOut, Save, Trash2 } from "lucide-react";
import type { Portfolio, Project, Experience } from "../../lib/portfolio";
...
export default function PortfolioEditor({ initial, signOutUrl }: { initial: Portfolio; signOutUrl: string }) {
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/portfolio", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(data) });
      const result = await response.json() as { error?: string; portfolio: Portfolio };
  ...
  return <main className="studio-page">
    <aside className="studio-sidebar"><a className="studio-brand" href="/"><span>AP</span><div>AMAN PAWAR<small>PORTFOLIO STUDIO</small></div></a><div className="studio-side-label">YOUR WORKSPACE</div><nav aria-label="Studio sections">{tabs.map(item => <button className={tab === item.id ? "active" : ""} key={item.id} onClick={() => setTab(item.id)}><span>{item.number}</span>{item.label}</button>)}</nav><div className="studio-side-bottom"><a href="/" target="_blank">View live site <ArrowUpRight size={16}/></a><a href={signOutUrl}>Sign out <LogOut size={16}/></a></div></aside>
    <div className="studio-main"><header className="studio-topbar"><a href="/"><ArrowLeft size={17}/> Portfolio</a><div><span className="studio-status" role="status">{status}</span><button onClick={save} disabled={busy}><Save size={16}/>{busy ? "Working…" : "Save changes"}</button></div></header>
      ...
      <div className="studio-bottom"><button onClick={save} disabled={busy}><Save size={16}/> Save changes</button><a href="/" target="_blank">Preview public portfolio <ArrowUpRight size={16}/></a></div>
      </div></div>
  </main>;
}
